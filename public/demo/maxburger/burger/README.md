...
O tenant de demonstração abre em `/?tenant=maxburger`. Após a identificação, o montador é oferecido uma vez por visita. Dois ícones à direita alternam burger montado (imagem estática) e camadas flutuantes. É um efeito **2,5D**, não geometria/WebGL nem rotação livre de 360°. O experimento geométrico removido anteriormente não foi reconstruído.

A prévia permanece fixa e centralizada enquanto as opções rolam. Trocar pão, receita ou ingredientes abre as camadas por 2,6 segundos; uma nova escolha reinicia esse intervalo e um gesto segurado pausa o fechamento. A montagem fecha em 650 ms e volta à imagem estática. A sombra de contato usa a mesma geometria na foto e na cena. Adicionar registra o pedido uma única vez, fecha visualmente o burger e o leva ao carrinho; movimento reduzido dispensa o voo.
...
Validação final desta integração: build de produção, 112 testes unitários e a suíte completa de 115 cenários de navegador passaram (1080 × 1920 com toque, 2 workers). Inclui as 120 combinações receita × pão × subconjunto de extras no resolvedor, os 15 pares receita/pão no showcase, drag out/in com recálculo, teclado após arrasto, reedição, cancelamento, estoque, fallback de asset, movimento reduzido, pagamento de demonstração e regressões de pizza/tenants. Os novos cenários cobrem centralização, rolagem nativa, renovação do fechamento automático, pausa enquanto o dedo está segurado, sombra e adição única durante o voo ao carrinho. O build mantém os avisos existentes de bundle acima de 500 kB e importação dinâmica também estática de `device-session`. Nenhum teste equivale a medição de 60 fps ou validação de alcance no aparelho físico.
